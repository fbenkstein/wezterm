...
# .github/installer/installer.command (committed to dot-github via Git LFS
# after local build/sign/notarize — see .github/installer/README.md) and
# places it next to WezTerm.app in the zip root.
#
# Produces <basename>+custom-install.zip in the current directory.
set -euo pipefail

APP_ZIP="${1:?Usage: $0 <app-zip>}"
INSTALLER=.github/installer/installer.command
...
install -m 755 "$INSTALLER" "$ZIPDIR/installer.command"
